...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    right.filterAcc(p, left.filterAcc(p, if (p.apply(elem)) acc.incl(elem) else acc))
  }
...
object GoogleVsApple {
  ...
  lazy val appleTweets: TweetSet = allTweets.filter(tweet => (apple.exists(elem => tweet.text.contains(elem))))
